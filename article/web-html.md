...
<!-- 在input标签中，name和id的意思都差不多，name就是名称咯类似于你的变量名，id是唯一标识这个标签，name和id在JS中都会用到，用来定位具体某个标签；但value含义就不同了要看你使用的是哪个类型的标签，譬如type=text文本框，value是显示在文本框中的初始语句；但是在type=radio单选框中，value的含义是你所选的值，然后提交给服务器的； -->
